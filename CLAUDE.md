...
### 2026-05-20 — 📏 UI 간격 측정: 코드 수치 ≠ 사용자 실측
**핵심 학습**: Tailwind 클래스 수치(px/rem)와 사용자가 모니터·휴대폰
화면을 실제 자로 잰 값은 일치하지 않을 수 있다. **사용자 실측이 항상 옳다.**

사례:
- `space-y-9` (이론 36px) + wrapper `pt-9` (이론 36px) = 합쳐서 72px 의도로
  옵션 카드 ↔ 히스토리 간격을 다른 영역의 약 2배로 만들려 했음.
- AI 의 스크린샷 픽셀 측정 결과: 1:1.44 (이미 차이 작음).
- **사용자 실측 결과(PC 모니터/iPhone): A·B 모두 약 75mm, 거의 동일.**

추정 원인(정확한 원인은 미상):
- 옵션 카드 안 내부 padding(`p-5 sm:p-6`)이 시각적으로 카드 외부 공간처럼
  인식되어 사용자 인식 시작점이 카드 보더가 아니라 카드 padding 끝에 가까움.
- 옵션 카드의 강한 보더가 시각적 무게라 외부 거리 인식을 압축.
- wrapper `pt` 가 padding 박스 안이라 사용자 인식상 효과가 적음.

**향후 UI 작업 규칙**:
1. AI 가 자체 스크린샷 픽셀 측정·이론 px 계산으로 단언하지 말 것. 사용자
   실측이 정답.
2. 보더 강한 카드 다음 영역의 간격은 코드 수치보다 더 크게 잡아야 시각적
   분리가 됨(경험칙).
3. 간격 미세 조정 시 코드 한 단계 변경 → 사용자 실측 피드백 → 추가 조정의
   짧은 루프를 활용.

### 2026-05-20 — 🎨 프리셋을 옵션 카드 밖으로 분리
- **결정**: 프리셋 칩을 옵션 카드 안에서 빼서 단독 행으로. 위치는 강도/위험
  영역과 옵션 카드 사이.
- **사유**: **프리셋과 옵션은 시각적 무게가 다름** — 프리셋은 "한 번 누르고
  끝"의 가벼운 액션, 옵션은 길이/카테고리/모드 등 미세 조정. 한 카드 안에
  묶으면 가벼운 액션이 무거운 영역에 갇혀 발견성이 떨어짐.
- **새 화면 순서**: 헤더 → 비번 카드 → 액션 → 강도/위험 → **프리셋(단독)** →
  옵션 카드(토글) → 히스토리 → 푸터.
- **카드/그림자 없음**: 가벼운 느낌 유지. 단독 chip 그룹.
- **편집 중 처리**: 프리셋이 fieldset 밖이라 fieldset disabled 영향 없음.
  PresetSelector wrapper 에 `opacity-50 pointer-events-none` 로 dim 처리.
- **옵션 카드 정리**: 프리셋 섹션과 그 아래 border-t 제거. 토글 button 의
  `border-t` 도 제거(첫 자식이 됨). `py-3.5` → `py-3` 으로 살짝 컴팩트해
  새 시각 균형.
- **Stage 1.3 결정의 반전**: 당시 "옵션 안 칩"으로 옮긴 건 페이지 상단의
  탭 메뉴 인상을 없애기 위해서였음. 이번 분리는 그 후 사용 데이터(편집·
  히스토리 추가 등 화면 정보량 증가) 누적 후 결정. 두 결정 모두 사용자
  피드백 기반의 자연스러운 진화.
...
- **증상**: 편집 모드 input 이 한글 입력을 그대로 받아 비번에 한글 섞임.
  대부분의 한국 사이트가 한글 비번을 거부하므로 실용성 0, 의도치 않은 부작용.
- **수정**: input `onChange` 에서 ASCII 인쇄 가능 범위(0x20–0x7E) 외 모든
  문자를 정규식으로 즉시 제거(`/[^\x20-\x7E]/g`). generator 의 문자 풀과
  완전히 일치. 보조로 `lang="en"`, `inputMode="text"` 힌트 추가(브라우저별
  IME 비활성화 효과 차이는 있지만 onChange 필터가 최종 차단).
- **UX**: 한글 키패드 상태에서 입력해도 화면에 아무것도 안 보이므로 사용자가
  즉시 한/영 전환 필요를 인지. 별도 토스트/배너 없이 단순함 유지.
- **Composition 의 `other` 카운트**: 필터로 차단되어 실제로는 0. 코드는
  안전망으로 유지(향후 ASCII 외 허용 모드 추가 가능성).
- **SPEC §6.2 갱신**: 입력 가능 문자를 ASCII로 한정한다는 점 명문화.
...
- **사유**:
  - 후보 B(`online_no_throttling_10_per_second`)로 가면 12자 비번이 "1조 년"
    수준의 비현실적 시간으로 표시됨. **요즘 GPU/ASIC 발전 속도를 고려하면
    오히려 과장**. 보안 도구가 사용자에게 거짓 안전감을 주는 게 위험.
  - 후보 C(score=4면 "사실상 무한")는 정보 손실.
  - A안은 정직함. "매우 강함 / 3년" 의 시각 충돌은 일부 있으나, zxcvbn 은
    오프라인 해시 노출 + 사전·패턴 디스카운트를 모두 반영한 **합리적 최악
    시나리오**. 보수적 표현이 차라리 옳다.
- **향후 트리거**: 사용자가 직접 "라벨이 헷갈린다"는 피드백을 가져오면 그때
  안내 한 줄 추가(후보 D 미니멀) 검토. 현재는 행동 없음.
